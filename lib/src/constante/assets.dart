class Assets {
  static const String pathAssetsImage = "assets/images/";
  static const String pathAssetsIcon = "assets/icon/";
  static const String pathAssetsGif = "assets/gif/";
  static const String pathAssetsJson = "assets/json/";
  Assets._();

  static const String radioOff = "${pathAssetsGif}radio_off.jpg";
  static const String radioOn = "${pathAssetsGif}radio_on.gif";
  static const String logoMIshapi = "${pathAssetsImage}logo_mishapi.jpg";
  static const String about = "${pathAssetsImage}about.png";
  static const String annonce = "${pathAssetsImage}ANNONCE.png";
  static const String appp = "${pathAssetsImage}appp.jpg";
  static const String contactradio = "${pathAssetsImage}contact_radio.png";
  static const String emission = "${pathAssetsImage}EMISSION.png";
  static const String icon = "${pathAssetsImage}icon.png";
  static const String micro = "${pathAssetsImage}micro.png";
  static const String notification = "${pathAssetsImage}notification.png";
  static const String pngwave = "${pathAssetsImage}png_wave.png";
  static const String top = "${pathAssetsImage}top_10.png";
  static const String logoRago = "${pathAssetsImage}RAGO_LOGO.png";
  static const String logoRagoS = "${pathAssetsImage}RAGO_LOGOs.png";
}
